...
class MyArrayList<E> implements Cloneable {
    ...
    public boolean addAll(Collection<? extends E> col){
        if(col == null) throw new UserNullPointerException();

        int newSize = col.size();
        if(newSize==0) return false ;
        // if (col.isEmpty()) return false;

        ensureCapacity(indx + newSize);
    ...
    // public boolean addAll(int indx, Collection<? extends E> col){
    //     if(col==null) throw new UserNullPointerException();
    //     if(indx<0 || indx>size()+1) throw new UserNoSuchElementException();
...
public class DriverExampleArrayList 
{
    public static void main(String[] args) throws CloneNotSupportedException 
    {
        ...
        System.out.println("list1 : "+ list1); // [10, 20, 30, 40, 50]
